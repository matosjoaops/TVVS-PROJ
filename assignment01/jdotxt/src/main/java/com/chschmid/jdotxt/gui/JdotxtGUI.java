...
public class JdotxtGUI extends JFrame {
	...
	private void initGUI() {
		...
		toolbar.getButtonSave().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				saveTasks(false);
			}
		});
		toolbar.getButtonReload().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				reloadTasks();
			}
		});
		toolbar.getButtonArchive().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				archiveTasks();
			}
		});
		toolbar.getButtonSettings().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				showSettingsDialog();
			}
		});
		...
		// toolbar.setVisibleSaveReload(!Jdotxt.userPrefs.getBoolean("autosave",
		// false));
		...
		MyMouseWheelListener myMouseWheelListener = new MyMouseWheelListener(tasksPane.getMouseWheelListeners(),
				SCROLL_AMOUNT);
		for (MouseWheelListener listener : tasksPane.getMouseWheelListeners())
			tasksPane.removeMouseWheelListener(listener);
		...
		if (Jdotxt.userPrefs.getBoolean("isMaximized", false))
			...
			public void windowClosing(WindowEvent e) {
				...
				if (taskBag.hasChanged()) {
					if (Jdotxt.userPrefs.getBoolean("autosave", false))
						...
						result = JOptionPane.showOptionDialog(JdotxtGUI.this, lang.getWord("Text_save_changes"),
								lang.getWord("jdotxt"), JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE,
								null, null, null);
						if (result == 0)
							Jdotxt.storeTodos();
					}
				...
				if (result < 2) {
					...
					Jdotxt.userPrefs.putBoolean("isMaximized",
							(JdotxtGUI.this.getExtendedState() & JFrame.MAXIMIZED_BOTH) != 0);
	...
	private void showReloadDialog() {
		...
		int result = JOptionPane.showOptionDialog(JdotxtGUI.this, lang.getWord("Text_modified"), lang.getWord("Reload"),
	...
	public void showSettingsDialog() {
		...
		} else if (currentCompactMode != newCompactMode)
			refreshGUI();
		if (currentAutoSave != autoSave) {
			if (!autoSave)
				taskList.removeTaskListener(autoSaveListener);
			else {
				saveTasks(true);
				taskList.addTaskListener(autoSaveListener);
			}
	...
	public static void loadLookAndFeel(String language) {
		...
		if (!fontR.getFamily().equals("Ubuntu Light") || !fontB.getFamily().equals("Ubuntu")) {
			try {
				fontR = Font.createFont(Font.TRUETYPE_FONT, Jdotxt.class.getResourceAsStream("fonts/Ubuntu-R.ttf"))
						.deriveFont(14f);
				fontRI = Font.createFont(Font.TRUETYPE_FONT, Jdotxt.class.getResourceAsStream("fonts/Ubuntu-MI.ttf"))
						.deriveFont(14f);
				fontB = Font.createFont(Font.TRUETYPE_FONT, Jdotxt.class.getResourceAsStream("fonts/Ubuntu-B.ttf"))
						.deriveFont(14f);
	...
	private short calculateVisibility() {
		short visibility = 0;
		if (Jdotxt.userPrefs.getBoolean("showProjectsPanel", true))
			visibility = JdotxtFilterPanel.VISIBILITY_PROJECTS;
		if (Jdotxt.userPrefs.getBoolean("showContextsPanel", true))
			visibility = (short) (visibility + JdotxtFilterPanel.VISIBILITY_CONTEXTS);
		return visibility;
	}
	...
	// File operations can be done in a separate thread, so that the GUI thread is
	// not blocked by file IO
		...
		public boolean dispatchKeyEvent(KeyEvent e) {
			if (e.getID() == KeyEvent.KEY_PRESSED) {
				...
				if (e.isControlDown() && e.getKeyCode() == KeyEvent.VK_F)
					toolbar.getTextfieldSearch().requestFocus(); // Jump to search bar
	...
	// After a task has been updated: update the filter panes (maybe some
	// projects/contexts have changed, or have been added/removed)
	private class FilterUpdateListener implements TaskListener {
		...
		@Override
		public void taskUpdated(Task t, short field) {
			if (field != JdotxtTaskPanel.CONTENT)
				filterPanel.updateFilterPanes();
		}
	...
	// After a task has been updated: Trigger saving to file if autosave is
	// activated.
	private class AutoSaveListener implements TaskListener {
		@Override
		public void taskCreated(Task t) {
			autoSaver.triggerAction();
		}
	...
	public class MyFilterChangeListener
			implements com.chschmid.jdotxt.gui.controls.JdotxtFilterPanel.FilterChangeListener {
		@Override
