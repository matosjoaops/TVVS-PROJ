...
public class JdotxtFilterPanel extends JPanel {
	...
	public void setSwitchPanels(boolean switchPanels) {
		if (this.switchPanels == switchPanels)
			return;
	...
	public boolean isFilterPaneUpToDate() {
		...
		if ((projects.getModel().getSize() != myProjects.size() + 2)
				|| (contexts.getModel().getSize() != myContexts.size() + 2))
			return false;

		for (int k1 = 0; k1 < myProjects.size(); k1++)
			if (!projects.getModel().getElementAt(k1 + 2).substring(1).equals(myProjects.get(k1)))
				return false;
		for (int k1 = 0; k1 < myContexts.size(); k1++)
			if (!contexts.getModel().getElementAt(k1 + 2).substring(1).equals(myContexts.get(k1)))
				return false;
	...
	private void forceUpdateFilterPanes() {
		...
		for (int k1 = 0; k1 < projectsString.length; k1++)
			if (selectedProjects.contains(projectsString[k1]))
				selectedProjectsIndices.add(k1);
		for (int k1 = 0; k1 < contextsString.length; k1++)
			if (selectedContexts.contains(contextsString[k1]))
				selectedContextsIndices.add(k1);

		if (selectedProjectsIndices.size() == 0)
			projects.setSelectedIndex(0);
		else
			projects.setSelectedIndices(Util.integerList2IntArray(selectedProjectsIndices));
		if (selectedContextsIndices.size() == 0)
			contexts.setSelectedIndex(0);
		else
			contexts.setSelectedIndices(Util.integerList2IntArray(selectedContextsIndices));
		...
		else if (selectedContextsIndices.size() == 0)
			contextsListener.forceValueChanged();
			...
			if (selection.contains(JdotxtGUI.lang.getWord("All"))
				...
				if (selection.size() > 1) {
					...
					for (int k1 = 2; k1 < all.getSize(); k1++) {
						if (!selection.contains(all.getElementAt(k1)))
							filter.add(all.getElementAt(k1).substring(1));
					}
					if (filter.size() == 0) {
						if (!selection.contains(JdotxtGUI.lang.getWord("Uncategorized")))
							filter.add("-");
						else
							filter.add("98jLpz+LfAH4JQ_lQJb0x"); // Make sure that no item is found when all items +
																	// "All" is selected
					}
				}
			} else {
