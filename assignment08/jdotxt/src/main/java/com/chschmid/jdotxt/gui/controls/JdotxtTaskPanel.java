...
public class JdotxtTaskPanel extends JPanel {
	...
	private void initLayout() {
		...
		textPriority.setMaximumSize(
				new Dimension(textPriority.getPreferredSize().width, textPriority.getPreferredSize().height));
		...
		textPriority.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				fireFocusLost(PRIORITY);
			}

			@Override
			public void focusGained(FocusEvent arg0) {
				fireFocusGained(PRIORITY);
			}
		});
		...
		textContent.getActionMap().put("PREV", new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				if (textContent.getCaretPosition() == 0)
		...
		textContent.getActionMap().put("NEXT", new AbstractAction() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (textContent.getCaretPosition() == textContent.getText().length() && !compactMode)
		...
		checkDone.addFocusListener(new FocusListener() {
			@Override
			public void focusLost(FocusEvent arg0) {
				fireFocusLost(COMPLETED);
			}

			@Override
			public void focusGained(FocusEvent arg0) {
				fireFocusGained(COMPLETED);
			}
		});
		...
		buttonDelete.addFocusListener(new FocusListener() {
			@Override
			public void focusLost(FocusEvent arg0) {
				fireFocusLost(DELETE);
			}

			@Override
			public void focusGained(FocusEvent arg0) {
				fireFocusGained(DELETE);
			}
		});
		...
		if (!compactMode)
			panelTodoInfo.add(textDate); // No-Date-mod
		...
		if (!compactMode)
			...
			panelTodoCommands.setLayout(new BoxLayout(panelTodoCommands, BoxLayout.X_AXIS));
		...
		if (isNewTask) {
			if (!compactMode)
				buttonNewTask = new JdotxtImageButton(Util.createImageIcon("/drawable/add.png"));
			else
				buttonNewTask = new JdotxtImageButton(Util.createImageIcon("/drawable/add_25.png"));
			...
			this.setBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, JdotxtGUI.COLOR_GRAY_PANEL));
		this.setBackground(Color.WHITE);

		if (isNewTask)
			this.add(buttonNewTask);
		else
		...
		am.put(DONE_ACTION, new AbstractAction() {
			@Override
			public void actionPerformed(ActionEvent e) {
				checkDone.setSelected(!checkDone.isSelected());
			}
		});
	...
	public void requestFocus(short control) {
		if (control == ADDNEW && buttonNewTask != null)
			buttonNewTask.requestFocus();
		if (control == PRIORITY)
	...
	private void fireTaskCreated() {
		...
		for (int i = taskListenerList.size() - 1; i >= 0; i--)
			taskListenerList.get(i).taskCreated(task);
	}

	private void fireTaskUpdated(short field) {
		// System.out.println("task updated (" + field +")");
		for (int i = taskListenerList.size() - 1; i >= 0; i--)
			taskListenerList.get(i).taskUpdated(task, field);
	}

	private void fireTaskDeleted() {
		// System.out.println("task deleted: ");
		for (int i = taskListenerList.size() - 1; i >= 0; i--)
			taskListenerList.get(i).taskDeleted(task);
	}

	private void fireEnterPressed(short field) {
		// System.out.println("enter (" + field +")");
		for (int i = taskListenerList.size() - 1; i >= 0; i--)
			taskListenerList.get(i).enterPressed(task, field);
	}

	private void fireFocusLost(short field) {
		// System.out.println("enter (" + field +")");
		for (int i = taskListenerList.size() - 1; i >= 0; i--)
			taskListenerList.get(i).focusLost(task, field);
	}

	private void fireFocusGained(short field) {
		// System.out.println("enter (" + field +")");
		for (int i = taskListenerList.size() - 1; i >= 0; i--)
			taskListenerList.get(i).focusGained(task, field);
	}
		...
		private void updateDate(DocumentEvent arg0) {
			if (JdotxtDateField.isValidDate(textDate.getText())) {
				if (!oldDate.equals(textDate.getText())) {
					...
					String rawText;
					if (oldDate.startsWith("-"))
						rawText = task.inFileFormatHeaderNoDate() + textContent.getText();
					else
						rawText = task.inFileFormatHeaderNoDate() + oldDate + " " + textContent.getText();
					task.update(rawText);
